...
class TokenService {
	...
	static getLocalToken = () => {
		return localStorage.getItem(this.tokenFieldName) || sessionStorage.getItem(this.tokenFieldName);
	};

	static getLocalRefreshToken = () => {
		return localStorage.getItem(this.refreshTokenFieldName) || sessionStorage.getItem(this.refreshTokenFieldName);
	};

	static getLocalRefreshTokenExpire = () => {
		return (
			localStorage.getItem(this.refreshTokenExpireFieldName) || sessionStorage.getItem(this.refreshTokenExpireFieldName)
		);
	};

	static getLocalTokenExpire = () => {
		return localStorage.getItem(this.tokenExpireFieldName) || sessionStorage.getItem(this.tokenExpireFieldName);
	};
	...
	static updateUser = (user: { avatar: string; username: string; id: string }) => {
	...
	static updateLocalToken = (token: string, expire: string, rememberMe: boolean = true) => {
		if (rememberMe) {
	...
	static updateLocalRefreshToken = (token: string, expire: string, rememberMe: boolean = true) => {
		if (rememberMe) {
	...
	static refreshTokens = async () => {
		const res = await ApiService.doRefresh(this.getLocalToken() as string, this.getLocalRefreshToken() as string);
		...
		if (res.status === 200) {
			...
			this.updateLocalToken(data.token, data.tokenExpire, this.findRememberMe());
			...
			this.updateLocalRefreshToken(data.refreshToken, data.refreshTokenExpire, this.findRememberMe());
